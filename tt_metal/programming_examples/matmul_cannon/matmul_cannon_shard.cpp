...
constexpr uint32_t DRAM_SHARD_M = 4; // each time load from DRAM/NoC: height
constexpr uint32_t DRAM_SHARD_N = 4; // each time load from DRAM/NoC: width
...
void matmul_cannon(std::vector<bfloat16>& a, std::vector<bfloat16>& b, std::vector<bfloat16>& output,
    ...
    // but only process DRAM_SHARD_M * DRAM_SHARD_N tiles at a time (not PER_CORE_M*PER_CORE_N), for the space in SRAM is so limited
    TT_ASSERT(Mt % PER_CORE_M == 0);
    TT_ASSERT(Nt % PER_CORE_N == 0);
    TT_ASSERT(Kt % PER_CORE_K == 0);
    TT_ASSERT(PER_CORE_M % DRAM_SHARD_M == 0);
    TT_ASSERT(PER_CORE_N % DRAM_SHARD_N == 0);
    TT_ASSERT(PER_CORE_K % DRAM_SHARD_K == 0);
    TT_ASSERT(DRAM_SHARD_M % SUBBLOCK_SIZE_H == 0);
    TT_ASSERT(DRAM_SHARD_N % SUBBLOCK_SIZE_W == 0);
    ...
    uint32_t in0_CB_tiles = DRAM_SHARD_M * DRAM_SHARD_K;
    uint32_t in1_CB_tiles = DRAM_SHARD_K * DRAM_SHARD_N;
    uint32_t out_CB_tiles = DRAM_SHARD_M * DRAM_SHARD_N;
    ...
    */
    ...
    auto reader_kernel_cannon = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/matmul_common/kernels/dataflow/reader_bmm_cannon_semaphore_shard.cpp",
        all_cores,
    ...
    auto writer_kernel_cannon = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/matmul_common/kernels/dataflow/writer_bmm_cannon_shard.cpp",
    ...
    std::vector<uint32_t> compute_compile_time_args = {
        ...
        (std::uint32_t) DRAM_SHARD_M,
        (std::uint32_t) DRAM_SHARD_N,
        (std::uint32_t) DRAM_SHARD_K
    };
    auto compute_kernel_cannon = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/matmul_common/kernels/compute/bmm_cannon_v3_shard.cpp",
