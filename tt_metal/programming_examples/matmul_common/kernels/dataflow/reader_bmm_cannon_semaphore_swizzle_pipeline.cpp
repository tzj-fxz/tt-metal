...
void kernel_main() {
    ...
    uint32_t l1_write_addr_dram0;
    uint32_t l1_write_addr_dram1;
    ...
    for (uint32_t nbatch = 0; nbatch < batch; ++nbatch) {
        ...
        uint32_t in0_start_addr_dram;
        uint32_t in1_start_addr_dram;
        uint32_t in0_start_addr_noc;
        uint32_t in1_start_addr_noc;
        // To pipelining DRAM and NoC, two set of CBs should be used in turn
        uint32_t noc_cb_A = tt::CB::c_in2;
        uint32_t noc_cb_B = tt::CB::c_in3;
        ...
        l1_write_addr_dram0 = get_write_ptr(dram_cb_A);
        in0_start_addr_dram = l1_write_addr_dram0;
        ...
        for (uint32_t h = 0; h < dram_shard_x; ++h) {
            for (uint32_t w = 0; w < per_core_K; ++w) {
                noc_async_read_tile(src0_start_tile_id + h * Kt + w, s0, l1_write_addr_dram0);
                l1_write_addr_dram0 += src0_tile_bytes;
            }
        }
        l1_write_addr_dram1 = get_write_ptr(dram_cb_B);
        in1_start_addr_dram = l1_write_addr_dram1;
        ...
        for (uint32_t h = 0; h < per_core_K; ++h) {
            for (uint32_t w = 0; w < dram_shard_y; ++w) {
                noc_async_read_tile(src1_start_tile_id + h * Nt + w, s1, l1_write_addr_dram1);
                l1_write_addr_dram1 += src1_tile_bytes;
            }
        }
        noc_async_read_barrier();
        cb_push_back(dram_cb_A, src0_sharded_tiles);
        cb_push_back(dram_cb_B, src1_sharded_tiles);
        dram_cb_A = tt::CB::c_in0;
        dram_cb_B = tt::CB::c_in1;
        in0_start_addr_noc = in0_start_addr_dram;
        in1_start_addr_noc = in1_start_addr_dram;

        for (uint32_t r = 1; r < round; ++r) {
            {
                cb_reserve_back(dram_cb_A, src0_sharded_tiles);
                cb_reserve_back(dram_cb_B, src1_sharded_tiles);
            ...
            for (uint32_t shift_num = 0; shift_num < Mt / per_core_M - 1; ++shift_num) {
                ...
                noc_semaphore_set(in1_sender_semaphore_addr_ptr, 0);
                noc_async_write(in0_start_addr_noc, src0_shift_to_addr_in0, src0_tile_bytes * dram_shard_x * per_core_K);
                ...
                noc_async_write(in1_start_addr_noc, src1_shift_to_addr_in1, src1_tile_bytes * per_core_K * dram_shard_y);
                noc_async_write_barrier();
                in0_start_addr_noc = l1_write_addr_in0;
                in1_start_addr_noc = l1_write_addr_in1;
