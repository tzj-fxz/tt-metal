...
void kernel_main() {
    ...
    for (uint32_t nbatch = 0; nbatch < batch; ++nbatch) {
        ...
        for (uint32_t r = 0; r < round; ++r) {
            cb_reserve_back(tt::CB::c_in2, src0_sharded_tiles);
            cb_reserve_back(tt::CB::c_in3, src1_sharded_tiles);
            ...
            if (swizzleA) {
                l1_write_addr_in2 = get_write_ptr(tt::CB::c_in2);
            ...
            if (swizzleB) {
                l1_write_addr_in3 = get_write_ptr(tt::CB::c_in3);
