...
void kernel_main() {
    ...
    uint32_t dram_shard_m = get_arg_val<uint32_t>(12);
    uint32_t dram_shard_n = get_arg_val<uint32_t>(13);
    uint32_t dram_shard_k = get_arg_val<uint32_t>(14);
    uint32_t subblock_h = dram_shard_m / subblock_size_h;
    uint32_t subblock_w = dram_shard_n / subblock_size_w;
    uint32_t subblock_tiles = subblock_size_h * subblock_size_w;

    constexpr bool out_is_dram = get_compile_time_arg_val(0) == 1;
    uint32_t output_tiles = dram_shard_m * dram_shard_n;
    ...
    for (uint32_t b = 0; b < batch; b++) {
        for (uint32_t dram_shard_h = 0; dram_shard_h < per_core_M / dram_shard_m; ++dram_shard_h) {
            for (uint32_t dram_shard_w = 0; dram_shard_w < per_core_N / dram_shard_n; ++dram_shard_w) {
                uint32_t output_shard_index = output_index + dram_shard_h * dram_shard_m * Nt + dram_shard_w * dram_shard_n;
