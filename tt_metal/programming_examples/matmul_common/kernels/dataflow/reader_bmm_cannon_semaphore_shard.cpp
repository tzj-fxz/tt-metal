...
void kernel_main() {
    ...
    uint32_t dram_shard_m = get_arg_val<uint32_t>(28);
    uint32_t dram_shard_n = get_arg_val<uint32_t>(29);
    uint32_t dram_shard_k = get_arg_val<uint32_t>(30);
    ...
    uint32_t src0_sharded_tiles = dram_shard_m * dram_shard_k;
    uint32_t src1_sharded_tiles = dram_shard_k * dram_shard_n;
    // the number of shard rows and shard cols, to determine swizzle rounds
    // make sure rounds is the multiple of 2
    uint32_t dram_shard_x = per_core_M / dram_shard_m;
    uint32_t dram_shard_y = per_core_N / dram_shard_n;
    uint32_t dram_shard_w = per_core_K / dram_shard_k;
    uint32_t swizzle_round = dram_shard_x * dram_shard_y * dram_shard_w;
    // TODO currenty batch = 1
    for (uint32_t nbatch = 0; nbatch < batch; ++nbatch) {
        // swizzle about DRAM load
        for (uint32_t r = 0; r < swizzle_round; ++r) {
            uint32_t a_shard_x = (r / dram_shard_y) % dram_shard_x;
            uint32_t a_shard_w = (r / dram_shard_y) / dram_shard_x;
            uint32_t b_shard_w = (r / (dram_shard_y * dram_shard_x));
            uint32_t b_shard_y = r % dram_shard_y;
                ...
                uint32_t in1_start_addr;
                {
                    ...
                    src0_start_tile_id += a_shard_x * dram_shard_m * Kt + a_shard_w * dram_shard_k;
                    for (uint32_t h = 0; h < dram_shard_m; ++h) {
                        for (uint32_t w = 0; w < dram_shard_k; ++w) {
                    ...
                    src1_start_tile_id += b_shard_w * dram_shard_k * Nt + b_shard_y * dram_shard_n;
                    for (uint32_t h = 0; h < dram_shard_k; ++h) {
                        for (uint32_t w = 0; w < dram_shard_n; ++w) {
                            // disable DRAM ops to check NoC and TFLOPS
                            // for (uint32_t t = 0; t < 32*32; ++t) {
                ...
                for (uint32_t shift_num = 0; shift_num < Mt / per_core_M - 1; ++shift_num) {
                    ...
                    noc_semaphore_set(in1_sender_semaphore_addr_ptr, 0);
                    noc_async_write(in0_start_addr, src0_shift_to_addr_in0, src0_tile_bytes * dram_shard_m * dram_shard_k);
                    ...
                    noc_async_write(in1_start_addr, src1_shift_to_addr_in1, src1_tile_bytes * dram_shard_k * dram_shard_n);
