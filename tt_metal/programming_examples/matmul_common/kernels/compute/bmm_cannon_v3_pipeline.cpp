...
void MAIN {
    ...
    uint32_t round = (per_core_M / dram_shard_x) * (per_core_N / dram_shard_y);

    uint32_t compute_cb_A;
    uint32_t compute_cb_B;
    ...
    for (uint32_t nb = 0; nb < batch; ++nb) {
        ...
        for (uint32_t r = 0; r < round; ++r) {
            // determine which cb to use
            if (r % 2 == 0) {
                compute_cb_A = tt::CB::c_in2;
                compute_cb_B = tt::CB::c_in3;
            } else {
                compute_cb_A = tt::CB::c_in0;
                compute_cb_B = tt::CB::c_in1;
            }
            mm_block_init(compute_cb_A, compute_cb_B, tt::CB::c_intermed0, false, subblock_size_w, subblock_size_h, per_core_K);
            ...
            for (uint32_t shift_num = 0; shift_num < num_block_x; ++shift_num) {
                {
                    cb_wait_front(compute_cb_A, in0_num_tiles);
                    cb_wait_front(compute_cb_B, in1_num_tiles);
                    ...
                    for (uint32_t subblock_m = 0; subblock_m < subblock_h; ++subblock_m) {
                        for (uint32_t subblock_n = 0; subblock_n < subblock_w; ++subblock_n) {
                            ...
                            if (enable_reload) {
                                copy_tile_to_dst_init_short_with_dt(compute_cb_B, tt::CB::c_intermed0);
                                ...
                                mm_block_init_short_with_dt(compute_cb_A, compute_cb_B, tt::CB::c_intermed0, false, subblock_size_w, subblock_size_h, per_core_K);
                            }
                    ...
                    cb_pop_front(compute_cb_A, in0_num_tiles);
                    cb_pop_front(compute_cb_B, in1_num_tiles);
                }
            }
        }
    }
