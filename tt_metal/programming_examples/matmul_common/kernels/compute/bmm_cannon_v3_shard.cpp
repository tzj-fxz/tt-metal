...
void MAIN {
    ...
    uint32_t dram_shard_m = get_compile_time_arg_val(9);
    uint32_t dram_shard_n = get_compile_time_arg_val(10);
    uint32_t dram_shard_k = get_compile_time_arg_val(11);
    uint32_t subblock_h = dram_shard_m / subblock_size_h; // in0_num_subblock
    uint32_t subblock_w = dram_shard_n / subblock_size_w; // in1_num_subblock
    ...
    mm_block_init(tt::CB::c_in0, tt::CB::c_in1, tt::CB::c_intermed0, false, subblock_size_w, subblock_size_h, dram_shard_k);
    DeviceZoneScopedN("TEST-bmm-cannon");

    uint32_t in0_num_tiles = dram_shard_m * dram_shard_k;
    uint32_t in1_num_tiles = dram_shard_k * dram_shard_n;
    uint32_t out_num_tiles = dram_shard_m * dram_shard_n;
    ...
    for (uint32_t nb = 0; nb < batch; ++nb) {
        ...
        // for (uint32_t dram_shard_h = 0; dram_shard_h < per_core_M / dram_shard_m; ++dram_shard_h) {
        //     for (uint32_t dram_shard_w = 0; dram_shard_w < per_core_N / dram_shard_n; ++dram_shard_w) {
                        ...
                        for (uint32_t subblock_m = 0; subblock_m < subblock_h; ++subblock_m) {
                            for (uint32_t subblock_n = 0; subblock_n < subblock_w; ++subblock_n) {
                                ...
                                if (enable_reload) {
                                    ...
                                    mm_block_init_short_with_dt(tt::CB::c_in0, tt::CB::c_in1, tt::CB::c_intermed0, false, subblock_size_w, subblock_size_h, dram_shard_k);
                                }

                                int dst_index = 0;
                                int in0_subblock_offset = subblock_m * dram_shard_k * subblock_size_h;
                                int in1_subblock_offset = subblock_n * subblock_size_w;

                                {
                                    // MATH(DeviceZoneScopedN("TEST-bmm-inner"));
                                    for (uint32_t inner_dim = 0; inner_dim < dram_shard_k; ++inner_dim) {
                                        matmul_block(
                                            tt::CB::c_in0,
                                            tt::CB::c_in1,
                                            in0_subblock_offset + inner_dim, // note: horizonal offset
                                            in1_subblock_offset + inner_dim * dram_shard_n, // note: vertical offset
                                            dst_index,
                                            false,
                                            subblock_size_w,
                                            subblock_size_h,
