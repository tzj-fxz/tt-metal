...
constexpr uint32_t PROFILING_ITERATIONS = 5;
...
void matmul_single_core(std::vector<bfloat16>& a, std::vector<bfloat16>& b, std::vector<bfloat16>& output, bool bcast_batch,
    ...
    */
    ...
    // the data is initial in DRAM or L1
    ...
    */
    auto reader_id = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/matmul_common/kernels/dataflow/reader_test.cpp",
        core,
        tt_metal::DataMovementConfig{.processor = DataMovementProcessor::RISCV_1, .noc = NOC::RISCV_1_default, .compile_args = reader_compile_time_args});

    auto writer_id = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/matmul_common/kernels/dataflow/writer_test.cpp",
        core,
    ...
    auto matmul_single_core_kernel_id = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/matmul_common/kernels/compute/bmm_block.cpp",
        core,
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        constexpr uint32_t N = TILE_WIDTH * tile_w;  // user-defined
        constexpr uint32_t K = TILE_WIDTH;  // user-defined
